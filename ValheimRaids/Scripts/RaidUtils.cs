...
namespace ValheimRaids.Scripts {
    internal class RaidUtils {
        public static bool RayCastStraight(Vector3 origin, Vector3 end, out RaycastHit raycastHit, LineRenderer render, int mask, float distance = Mathf.Infinity) {
            var direction = new Vector3(end.x - origin.x, 0, end.z - origin.z);
            var start = new Vector3(origin.x, origin.y + 0.5f, origin.z);

            bool hit = Physics.Raycast(start, direction, out RaycastHit rayHit, distance, mask);
